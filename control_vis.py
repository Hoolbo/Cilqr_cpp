...
def read_cilqr_data(data_folder):
    """
    Read all CILQR JSON files from specified folder
    ...
    # Get all cilqr_data_*.json files
    json_files = []
    for filename in os.listdir(data_folder):
        if filename.startswith('cilqr_data_') and filename.endswith('.json'):
            # Extract file number for sorting
            try:
                file_num = int(filename.replace('cilqr_data_', '').replace('.json', ''))
    ...
    print(f"Found {len(json_files)} CILQR data files")
...
def plot_cilqr_data(time_steps, velocities, gammas):
    """
    ...
    ax1.set_title('CILQR Data Analysis - Velocity and Articulation Rate Over Time')
